...
(function (root) {
  ...
  function ask() {
    ...
    fetch('/api/plan-assistant', {
      ...
      body: JSON.stringify({ page: '10', instruction: text, plan: plan, examples: examples })
    }).then(function (r) {
  ...
  var CSS = [
    ...
    '#sa-clip{background:#e2e8f0;border:0;border-radius:8px;padding:9px 11px;font-size:15px;cursor:pointer}',
    '#sa-clips{flex-wrap:wrap;gap:5px;padding:6px 8px 0}',
    '.sa-clip{display:inline-flex;align-items:center;gap:5px;background:#dbeafe;color:#1e3a5f;',
    '  border-radius:12px;padding:3px 6px 3px 9px;font-size:11px;font-weight:600;max-width:150px;',
    '  overflow:hidden;white-space:nowrap}',
    '.sa-clip button{background:none;border:0;color:#1e3a5f;font-size:14px;cursor:pointer;padding:0 2px}'
  ].join('');
  ...
  var HTML =
    ...
    '<div id="sa-panel">' +
      ...
      '<div id="sa-clips" style="display:none"></div>' +
      '<div id="sa-ask">' +
        '<button type="button" id="sa-clip" title="Attach an example picture">\ud83d\udcce</button>' +
  ...
  function boot() {
    ...
    el('sa-clip').addEventListener('click', pickExamples);
